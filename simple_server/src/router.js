...
router.get('/', async (req, res) => {
  try {
    res.send({ message: 'Welcome to the Easy Store API!'} );
  } catch (error) {
...
router.get('/delete/:objectKey', async (req, res) => {
  try {
    const objectKey = req.params.objectKey;
    const url = await s3.generateDeleteURL(process.env.S3_BUCKET, objectKey, 180);
    res.send({ url })
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
